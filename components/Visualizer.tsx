...
const Visualizer: React.FC<VisualizerProps> = ({ text, isLoading }) => {
  return (
    <div className="glass rounded-3xl p-8 min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden group">
      ...
      <div className="absolute top-0 left-0 w-32 h-32 bg-amber-500/10 blur-3xl rounded-full -translate-x-1/2 -translate-y-1/2 group-hover:bg-amber-500/20 transition-all duration-700"></div>
      <div className="absolute bottom-0 right-0 w-32 h-32 bg-yellow-500/10 blur-3xl rounded-full translate-x-1/2 translate-y-1/2 group-hover:bg-yellow-500/20 transition-all duration-700"></div>
      ...
      {isLoading ? (
        <div className="space-y-4 text-center">
          ...
          <p className="text-amber-500/70 font-light italic">正在思考文字的可能性...</p>
        </div>
      ) : (
        <div className="relative z-10 text-center space-y-8 animate-in fade-in zoom-in duration-500">
          <svg className="w-12 h-12 text-amber-900/50 mx-auto mb-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-2xl md:text-3xl font-light leading-relaxed text-amber-50 max-w-sm mx-auto">
            {text}
          </p>
          ...
          <div className="pt-8 flex flex-wrap justify-center gap-4">
            <button 
              ...
              className="text-xs uppercase tracking-widest text-amber-600 hover:text-amber-200 transition-colors border-b border-amber-900/50 hover:border-amber-400 pb-1"
            >
              複製文字
            </button>
            <span className="text-amber-900">|</span>
            <button className="text-xs uppercase tracking-widest text-amber-600 hover:text-amber-200 transition-colors border-b border-amber-900/50 hover:border-amber-400 pb-1">
              分享靈感
            </button>
          </div>
        </div>
